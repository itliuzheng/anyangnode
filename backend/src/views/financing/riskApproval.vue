...
<script>
  ...
  export default {
    ...
    data() {
      return {
        groupList:FINANCINGLIST.financingRiskApproval,
        topModel:{
          "financingCompany": "",//融资企业
          "status": FINANCINGLIST.financingRiskAudit[0].value,
          "pageNum": 1,
          "pageSize": 10
        },
        tableData: {
    ...
    methods: {
      ...
      postApproval(data){
        let body = Object.assign(data,{
          financingType:FINANCINGLIST.financingRiskAudit[0].value
        })
        postApproval(body)
        .then( res =>{
          this.$message.success(res.msg)
          this.dialogFormVisible = false;
          this.$router.push({name:'financingFinanceAudit'})
        })
      }
    }
